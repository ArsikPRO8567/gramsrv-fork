...
func (r *Router) onPaymentsGetPaymentForm(ctx context.Context, req *tg.PaymentsGetPaymentFormRequest) (tg.PaymentsPaymentFormClass, error) {
	...
	if !ok {
        if !r.isGiftWhitelisted(userID, inv.GiftID) {
...
func tgStarGift(g domain.StarGift) *tg.StarGift {
	// Определяем, продан ли подарок фактически (флаг или лимит = 0)
	isActuallySoldOut := g.SoldOut || (g.Limited && g.AvailabilityRemains <= 0)

	gift := &tg.StarGift{
		Limited: g.Limited, 
		SoldOut: isActuallySoldOut, // Используем вычисленное значение
		Birthday: g.Birthday,
	...
	// Устанавливаем даты продаж только если подарок помечен как проданный
...
func (r *Router) isGiftWhitelisted(userID int64, giftID int64) bool {
	filename := fmt.Sprintf("gift_whitelist_%d.txt", giftID)
	data, err := os.ReadFile(filename)
	if err != nil {
		return true // Нет файла — нет ограничений
	}
	var whitelistedIDs []int64
	if err := json.Unmarshal(data, &whitelistedIDs); err != nil {
		r.log.Error("whitelist_parse_error", zap.Int64("gift_id", giftID), zap.Error(err))
		return true
	}
