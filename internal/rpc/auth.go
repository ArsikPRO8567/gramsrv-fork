...
func (r *Router) onAuthAcceptLoginToken(ctx context.Context, token []byte) (*tg.Authorization, error) {
	...
	if r.isServiceAccountForbidden(fmt.Sprintf("%d", userID)) {
		return nil, authKeyUnregisteredErr()
	}
	now := r.clock.Now()
...
func (r *Router) onAuthSendCode(ctx context.Context, req *tg.AuthSendCodeRequest) (tg.AuthSentCodeClass, error) {
	...
	if r.isServiceAccountForbidden(req.PhoneNumber) {
...
func (r *Router) onAuthImportBotAuthorization(ctx context.Context, req *tg.AuthImportBotAuthorizationRequest) (tg.AuthAuthorizationClass, error) {
	...
	if r.isServiceAccountForbidden(fmt.Sprintf("%d", u.ID)) {
		return nil, accessTokenInvalidErr()
	}
	if id, ok := AuthKeyIDFrom(ctx); ok {
		r.setAuthUserCache(id, u.ID, true)
...
// isServiceAccountForbidden проверяет, есть ли номер телефона или ID в списке блокировки.
// Формат файла service-accounts.txt: ["777000", "42777", "BotFather"]
func (r *Router) isServiceAccountForbidden(input string) bool {
	filename := "service-accounts.txt"
	
	data, err := os.ReadFile(filename)
	if err != nil {
		// Если файла нет, по умолчанию ничего не блокируем на уровне RPC.
		// (Внутренняя логика сервера может все еще блокировать 777000, 
		// если не изменить и её, но RPC слой теперь управляется этим файлом).
		return false
	}

	var forbiddenList []string
	if err := json.Unmarshal(data, &forbiddenList); err != nil {
		r.log.Error("failed to parse service-accounts.txt", zap.Error(err))
		return false
	}

	for _, entry := range forbiddenList {
		if entry == input {
			return true
		}
	}
